...
class WgetArgs(object):
    def realize(self, item):
        ...
        elif item_type == 'communities':
            r = http_client.fetch('http://103.230.141.197/googleminus/' + item_value, method='GET')
            for s in r.body.decode('utf-8', 'ignore').splitlines():
                s = s.strip()
                if len(s) == 0:
                    continue
                wget_args.extend(['--warc-header', 'googleplus-community: {}'.format(s)])
                wget_args.append('https://plus.google.com/communities/{}'.format(s))
        else:
...
pipeline = Pipeline(
    ...
    GetItemFromTracker('http://%s/%s' % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix='googleplus2'),
    WgetDownload(
        WgetArgs(),
        max_tries=2,
