...
allowed = function(url, parenturl)
  if string.match(url, "'+")
      ...
      or string.match(url, "^https?://[^/]*googleusercontent%.com/proxy/")
      or string.match(url, "^https?://[^/]*archive%.org/") then
    return false
  end
...
wget.callbacks.get_urls = function(file, url, is_css, iri)
  ...
  if string.match(url, "^https?://plus%.google%.com/[0-9]+$") and item_type == "users" then
    users[string.match(url, "([0-9]+)$")] = true
  elseif string.match(url, "^https?://plus%.google%.com/communities/[0-9]+$") and item_type == "communities" then
    users[string.match(url, "([0-9]+)$")] = true
  end

  if allowed(url, nil) and not string.match(url, "^https?://[^/]*googleusercontent%.com/") then
    html = read_file(file)
    if string.match(url, "^https?://plus%.google%.com/[0-9]+$")
        or string.match(url, "^https?://plus%.google%.com/communities/[0-9]+$") then
      if string.match(html, '<link%s+rel="canonical"%s+href="https?://plus%.google%.com/[^/"]+">')
          and item_type == "users" then
      ...
      elseif string.match(html, '<span%s+class="RveJvd%s+snByac">View%s+all</span>')
          and item_type == "users" then
        check(url .. "/palette")
      end
      local sid = string.match(html, '"FdrFJe":"([^"]+)"')
      local version = string.match(html, '"cfb2h":"([^"]+)"')
      local user_id = string.match(url, "([0-9]+)$")
      local current_time = os.date("*t")
      local reqid = current_time.hour * 3600 + current_time.min * 60 + current_time.sec
      local data = nil
      if item_type == "users" then
        data = string.match(html, "AF_initDataCallback%({key:%s+'ds:6'.-return%s*(.-)}}%);</script>")
      elseif item_type == "communities" then
        data = string.match(html, "AF_initDataCallback%({key:%s+'ds:7'.-return%s*(.-)}}%);</script>")
      end
      ...
      local extension = "74333095"
      if item_type == "communities" then
        extension = "77894468"
      end
      if data[1][2] ~= nil then
        local newurl = "https://plus.google.com/_/PlusAppUi/data?ds.extension=" .. extension .. "&f.sid=" .. sid .. "&bl=" .. version .. "&hl=en-US&soc-app=199&soc-platform=1&soc-device=1&_reqid=" .. reqid .. "&rt=c"
        local post_data = 'f.req=[[[' .. extension .. ',[{"' .. extension .. '":["' .. data[1][2] .. '","' .. user_id .. '"]}],null,null,0]]]'
        if item_type == "communities" then
          post_data = 'f.req=[[[' .. extension .. ',[{"' .. extension .. '":["' .. user_id .. '",null,"' .. data[1][2] .. '",null,null,null,null]}],null,null,0]]]'
        end
    ...
    if string.match(url, "^https?://plus%.google%.com/communities/[0-9]+$") then
      local commid = string.match(url, "([0-9]+)$")
      for commid_, catid in string.gmatch(html, 'data%-communityid="([^"]+)"%s+data%-categoryid="([^"]+)"') do
        if commid_ == commid then
          check(url .. "/stream/" .. catid)
        end
      end
    end
